...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('vars.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
...
<div class="d-flex flex-column mb-7 fv-row col-sm-12">
    <div class="form-group">
        ...
        <input type="file" class="dropify" name="image" data-default-file="{{get_file($row->media)}}" accept="image/*"/>
        <span
    ...
    <div class="d-flex flex-column mb-7 fv-row col-sm-12">
        ...
        <input id="video"  type="text" class="form-control form-control-solid"
                placeholder="" name="video" value="{{$video->media}}"/>   
    </div>
    @endforeach
 ...
 @foreach(languages() as $index=>$language)
   ...
   <div class="d-flex flex-column mb-7 fv-row col-sm-12">
       ...
       <!--end::Label-->
       <textarea name="desc[{{ $language->abbreviation }}]" id="{{'desc_'.$language->abbreviation }}" class="form-control" rows="5" placeholder="">{{ $row->getTranslation('desc', $language->abbreviation) }}</textarea>
