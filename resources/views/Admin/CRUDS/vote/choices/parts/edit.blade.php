...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('vote-choice.update',$data['choice']->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        <div class="form-group">
            ...
            <input type="file" class="dropify" name="image" data-default-file="{{get_file($data['choice']->image)}}" accept="image/*"/>
            <span
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <input id="title{{$language->abbreviation}}" required type="text" class="form-control form-control-solid"
                    placeholder="" name="title[{{$language->abbreviation}}]" value="{{$data['choice']->title}}"/>
        </div>
        @endforeach     
        ...
        <select name="vote_id" class="form-control form-control-solid">
            ...
            <option @if($data['choice']->vote_id == $vote->id) selected @endif value="{{ $vote->id }}">{{ $vote->title }}</option>
            @endforeach
        </select>
        </div> 
