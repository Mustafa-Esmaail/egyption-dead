...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route("products.store")}}">
    ...
    <div class="row g-4">
        ...
        <!-- <div class="d-flex flex-column mb-7 fv-row col-sm-6">
         ...
         @foreach(languages() as $index=>$language)
           ...
           @foreach(languages() as $index=>$language)
            ...
            <div class="d-flex flex-column mb-7 fv-row col-sm-6">
               <!--begin::Label-->
               <label for="address{{$language->abbreviation}}" class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
                   <span class="required mr-1">{{helperTrans('admin.address')}}({{$language->abbreviation}})</span>
                   <span class="red-star">*</span>
               </label>

               <!--end::Label-->
               <input id="address{{$language->abbreviation}}" required type="text" class="form-control form-control-solid"
                      placeholder="" name="address[{{$language->abbreviation}}]" value=""/>
           </div>
            @endforeach --}}
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
               ...
               <!--end::Label-->
               <input type="number" name="price" value="" id="price" required class="form-control form-control-solid"
                      placeholder=""  />
        </div>
